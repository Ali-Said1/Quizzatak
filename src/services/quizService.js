...
const quizService = {
  ...
  async updateGameSession(gameSessionId, updateData) {
    try {
      const { data } = await api.patch(`/game-sessions/${gameSessionId}`, updateData);
      return data.gameSession ?? data;
    } catch (error) {
  ...
  async getSubmissionsForGame(gameSessionId) {
    try {
      const { data } = await api.get(`/submissions?gameSessionId=${gameSessionId}`);
  ...
  async getSubmissionForStudent(gameSessionId, studentId) {
    try {
      const { data } = await api.get(`/submissions?gameSessionId=${gameSessionId}&studentId=${studentId}`);
  ...
  async hasAnsweredQuestion(gameSessionId, studentId, questionId) {
    try {
      const { data } = await api.get(`/submissions?gameSessionId=${gameSessionId}&studentId=${studentId}`);
      const submission = Array.isArray(data) ? data[0] : data.submission ?? data;
      if (!submission) return false;
      return submission.answers?.some(a => a.questionId === questionId) ?? false;
    } catch (error) {
